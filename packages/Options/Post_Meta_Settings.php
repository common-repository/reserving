...
class Post_Meta_Settings extends Base_Settings {
	...
	public function save( $post_id ) {
        ...
        if ( ! isset( $_POST['reserving_inner_custom_box_nonce'] ) ) {
            return $post_id;
        }
  
        $nonce = sanitize_text_field($_POST['reserving_inner_custom_box_nonce']);
  
        if ( ! wp_verify_nonce( $nonce, 'reserving_inner_custom_box' ) ) {
            return $post_id;
        }
    ...
    public function render_meta_box_content( $post ) {
        ...
        wp_nonce_field( 'reserving_inner_custom_box', 'reserving_inner_custom_box_nonce' );
		...
		foreach($fields as $key => $field){
			...
			if( isset( $field[ 'type' ] ) ) {
				...
				if( class_exists($field_class) ) {
					...
					$label = isset($field['label']) && $field['label'] == false ? false : true;
					$new_bj->render_field($label);
