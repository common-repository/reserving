...
class Settings {
		...
		foreach ( $this->parent->sections as $section ) {
			...
			if( isset($section['type']) && ( 'blank' == $section['type'] || 'url' == $section['type'] ) ) {
				if( !empty($section['callback']) ) {
					add_action( 'reserving_options_page_' . $this->parent->configs['opt_name'] . '_' . $section['id'], $section['callback'] );
				}
				continue;
			}
			...
			if( isset($section['fields']) && !empty($section['fields']) ) {
				foreach ( $section['fields'] as $key => $option ) {
					...
					$option['option_id']   = '';
					$option['class']       = isset($option['heading'] ) && $option['heading'] == false ? 'reserving-option-no-heading '. $option['type'] : $option['type'];
	...
	public function field_callback( $field ) {
		...
		if( isset($field['type']) ) {
			...
			if( class_exists($field_class) ) {
				...
				$label  = isset($field['label']) && $field['label'] == true ? true : false;
				$render->render_field($label);
			} else {
				_e('This field type does not exists, please check your code.', 'reserving');
			}
	...
	public function sanitize_callback( $options = array() ) {
        ...
        $sanitize_data = sanitize_text_field( $_POST['_wp_http_referer'] );
		parse_str( $sanitize_data , $referrer );
		...
		foreach ($options as $key => $value) {
			...
			if( $type ) {
				...
				$value = apply_filters( "reserving_settings_sanitize_{$this->parent->configs['opt_name']}[{$key}]", $value );
			}

			//General filter
			$sanitizied_options[ $key ] = apply_filters('reserving_settings_sanitize', $value );
		}
		...
		add_settings_error( "reserving-". $this->parent->configs['opt_name'] ."-notices", '', __( 'Settings updated!', 'reserving' ), 'updated' );
	...
	public function reset_section_options() {
		...
		if($_GET['action'] == 'reset' && wp_verify_nonce( $_GET['reset_nonce'], 'reserving_reset')) {
			$_tab = sanitize_text_field( $_GET['tab'] );
			$section = $this->parent->sections[$_tab];
			$new_options = array();
			...
			add_settings_error( "reserving-". $this->parent->configs['opt_name'] ."-notices", '', __( 'Section Defaults Restored!', 'reserving' ), 'updated' );
			$this->parent->refresh();
		}
	}
	 ...
	 * @return void
	 */
	public function load_field_dependency() {
	...
	public function admin_messages() {
		settings_errors( "reserving-". $this->parent->configs['opt_name'] ."-notices" );
	}
}
?>