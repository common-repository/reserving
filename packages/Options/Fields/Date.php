...
class Date extends Base_Field {
	...
	public function render_field($label = false) {
		...
		if($label):
			echo wp_kses_post(sprintf('<div class="ray-form-field option-date-wrapper"><label class="ray-option-label"> %s </label>', esc_html($this->field['title'])));
		endif; 

	?>
<div class="reserving-date-picker-option">
    <input type="date" class="<?php echo esc_attr($class); ?>" name="<?php echo esc_attr($this->option_name); ?>"
