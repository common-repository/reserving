...
namespace Reserving\system\config;
...
final class Loader
{
    ...
    public function store_array_config($dir){
        ...
        if( is_dir( $dir ) ){
            ...
            $files       = reserving_get_dir_file_list($dir);
            ...
            foreach($files as $key => $file_path){
                ...
                if(file_exists($file_path)){
                    ...
                    if( is_array( $array_content ) ) {
                          ...
                          reserving_app()->add($key, new \Reserving_Packages\DI\Argument\Literal\ArrayArgument($array_content));
                    }
                }
            ...
            if(is_array($file_arrs)){
                reserving_app()->add( 'builder-options-'.$base_folder , $file_arrs );
            }
    ...
    public function store_json_config( $dir ){
        ...
        if( is_dir( $dir ) ){
            ...
            $files  =  reserving_get_dir_file_list($dir,'json');
            foreach($files as $key => $file_path){
