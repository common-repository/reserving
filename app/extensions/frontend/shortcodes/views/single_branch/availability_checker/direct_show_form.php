...
<div class="form-popup reserving-form-popup single_branch direct_show" id="availabilityChecker">
    <?php if ($checker_form_title_show): ?>
        <h3><?php echo esc_html($checker_form_title); ?></h3>
    <?php endif; ?>
    <div class="shop_page reserving__tabs">
        <?php if ($deliery_activated) { ?>
            <button class="tablinks" onclick="openTabContent(event, 'deliveryChecker')"><?php echo esc_html($delivery_label); ?></button>
        <?php } ?>

        <?php if ($pickup_activated) { ?>
            <button class="tablinks" onclick="openTabContent(event, 'pickupChecker')"><?php echo esc_html($pickup_label); ?></button>
        <?php } ?>

        <?php if ($inrest_activated) { ?>
            <button class="tablinks" onclick="openTabContent(event, 'inRestChecker')"><?php echo esc_html($in_rest_label); ?> </button>
        <?php } ?>
